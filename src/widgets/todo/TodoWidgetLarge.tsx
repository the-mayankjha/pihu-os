...
import { Check, Trash2, Plus, Star, Menu } from 'lucide-react';
...
export interface TodoWidgetLargeProps {
  preview?: boolean;
  onClick?: () => void;
}
...
export const TodoWidgetLarge: React.FC<TodoWidgetLargeProps> = ({ preview = false, onClick }) => {
  ...
  // Tabs counts
  const todayCount = todos.filter(t => t.time && t.time.includes('AM') || t.time?.includes('PM') || t.time?.includes('Today')).length;
  const upcomingCount = todos.filter(t => t.time?.includes('Tomorrow')).length;

  const filteredTodos = todos.filter(todo => {
    if (activeTab === 'All') return true;
    if (activeTab === 'Completed') return todo.completed;
    if (activeTab === 'Today') return todo.time && (todo.time.includes('AM') || todo.time.includes('PM') || todo.time.includes('Today'));
    if (activeTab === 'Upcoming') return todo.time && todo.time.includes('Tomorrow');
    return true;
  });
  ...
  const innerContent = (
    <GlassCard
      ...
      style={{ backgroundColor: 'rgba(15, 23, 42, 0.4)' }} // Darker base to match mockup
    >
      {/* Header */}
      ...
      {/* Tabs */}
      <div className="flex items-center gap-6 mb-6 overflow-x-auto custom-scrollbar pb-1 cursor-auto text-sm">
        {(['All', 'Today', 'Upcoming', 'Completed'] as TabType[]).map((tab) => {
          ...
          return (
            <button
              ...
              style={{ color: isActive ? theme.colors.primary : 'rgba(255,255,255,0.5)' }}
            >
              <span className={`font-medium ${isActive ? 'bg-clip-text text-transparent' : ''}`} style={isActive ? { backgroundImage: `linear-gradient(to right, ${theme.colors.primary}, #FF6B6B)` } : {}}>
                {tab}
              </span>
              <span 
                className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold"
                style={{ 
                  backgroundColor: isActive ? theme.colors.primary : 'rgba(255,255,255,0.1)',
        ...
        <AnimatePresence initial={false}>
          {filteredTodos.map((todo) => (
            <motion.div
              ...
              className="group flex items-start gap-4 py-4 border-b border-white/5 relative"
            >
              <button 
                ...
                <div className="flex items-center gap-2 flex-wrap mt-1">
                  ...
                  {todo.tags?.map(tag => {
                    ...
                    return (
                      <span 
                        key={tag}
                        className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-md w-fit"
                        style={{ 
                  ...
                  {/* Fallback for old tasks that only have category */}
                  {!todo.tags?.length && todo.category && (
                    <span 
                      className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-md w-fit"
                      style={{ 
                        backgroundColor: `${todo.categoryColor || theme.colors.primary}20`,
                        color: todo.categoryColor || theme.colors.primary
                      }}
                    >
                      {todo.category}
                    </span>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <button>
                    <Star 
                      size={16} 
                      fill={todo.isStarred ? theme.colors.primary : "transparent"} 
                      color={todo.isStarred ? theme.colors.primary : "rgba(255,255,255,0.4)"} 
                    />
                  </button>
                </div>
              </div>
  ...
  if (preview) {
    ...
    return (
      ...
      >
        <div style={{ transform: 'scale(0.5)', width: 380, height: 520 }} className="flex-shrink-0 origin-center flex items-center justify-center pointer-events-none">
           {innerContent}
        </div>
      </div>
    );
